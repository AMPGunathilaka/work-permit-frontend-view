function fetchAllCompletedWorkPermitsByAP() {
    $.ajax({
        method: "GET",
        url: BASE_URL+"/api/v1/completionWorkPermitByAp/getAllCompletedWorkPermitByAp",
        success: function (data) {
            ...
            data.forEach(function (workPermit, index) {
                ...
                $('#completionByApWorkPermits').append(`
                    <tr>
                        ...
                        <td><button id="confirmCompletionHse" class="btn btn-success approve-btn" data-appointment-id="${workPermit.workPermitId}">Confirm - HSE</button></td>
                    </tr>
                `);
            });
        },
  ...
  function updateWorkPermitWhenCompletionByHSE(selectedWorkPermitId) {
    ...
    $.ajax({
        ...
        success: function(response) {
            var completedByHSEInCharge = response.firstName + " "+ response.lastName;
            var hseCompletedDate = new Date();
            
             $.ajax({
                method: "PUT", 
                url: BASE_URL+"/api/v1/completionWorkPermitByHse/update", 
                contentType: "application/json", // Set content type to JSON
                data: JSON.stringify({
                    workPermitId: selectedWorkPermitId,
                    hseCompletedDate: hseCompletedDate,
                    completedByHSEInCharge: completedByHSEInCharge
